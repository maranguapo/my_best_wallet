...
from src.gerar_pdf import CsvParaPdf
from src.modelos import MagicForm, ModelBazin, ModelGrahan
from src.modelos.schemas import GetTicketsRequest
...
def get_tickets(request: GetTicketsRequest):
    logging.info(f"Entrou na função")
    tipo_papel = request.ticketType
    ...
    try:
        ...
        logging.info(f"Lista de Papéis: {lista_papeis}")
        return lista_papeis
    except Exception as e:
        logging.error(f"Erro: {e}")
        raise HTTPException(status_code=500, detail="Erro ao processar o request")
